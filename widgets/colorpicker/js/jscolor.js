	...
	var jsc = {
		...
		getPadYComponent : function (thisObj) {
			switch (thisObj.mode.charAt(1).toLowerCase()) {
				case 'i': return 'i'; break; case 'v': return 'v'; break;
			}
			return 's';
		},
		...
		setPad : function (thisObj, e, ofsX, ofsY) {
			...
			switch (jsc.getPadYComponent(thisObj)) {
				...
				case 'v':
					...
					break;
				case 'i':
					if (thisObj.gamutZoom) {
						var xy = huepi.HelperZoomXYforModel(x / (thisObj.width - 1),  1 - (y / thisObj.height), thisObj.gamut);
						thisObj.fromXY(xy[0], xy[1], thisObj.xy[2], jsc.leaveSld);
					} else {
						thisObj.fromXY(x / (thisObj.width - 1), 1 - (y / thisObj.height), thisObj.xy[2], jsc.leaveSld);
					}
					break;
			}
		},
		...
		setSld : function (thisObj, e, ofsY) {
			...
			switch (jsc.getSliderComponent(thisObj)) {
				...
				case 'v':
					...
					break;
				case 'e':
					thisObj.fromXY(thisObj.xy[0], thisObj.xy[1], (yVal/100), jsc.leavePad);
					break;
		...
		createPalette : function () {
			...
			var drawFunc = function (width, height, type, gamut, gamutZoom) {
				canvas.width = width;
				canvas.height = height;

				ctx.clearRect(0, 0, canvas.width, canvas.height);

				if (type.toLowerCase() == 's' || type.toLowerCase() == 'v') {
					...
					for (var x = 0; x < canvas.width; x++) { // Every Pixel on ScanLine
						for (var y = 0; y < canvas.height; y++) { // Every ScanLine
							...
							var xy;
							if (gamutZoom){
								xy = huepi.HelperZoomXYforModel(x / canvas.width, 1 - (y / canvas.height), gamut);
							}else {
								xy = [
									x / canvas.width,
									1 - (y / canvas.height)
							...
							var XYColor = huepi.HelperXYtoRGBforModel(xy[0], xy[1], 1, gamut);
		...
		jscolor : function (targetElement, options) {
			...
			this.fromXY = function (x, y, b, flags) { // null = don't change
				...
				this.fromRGB(Math.round(cieColor.Red * 255), Math.round(cieColor.Green * 255), Math.round(cieColor.Blue*255), jsc.leaveSld);
				this.exportColor(flags);
			};
			...
			function drawPicker () {
				...
				p.padPal.draw(THIS.width, THIS.height, jsc.getPadYComponent(THIS), THIS.gamut, THIS.gamutZoom);
			...
			function redrawPad () {
				...
				switch (jsc.getPadYComponent(THIS)) {
					...
					case 'i': var yComponent = 3; break;
				}
				if (yComponent == 3){
					if (THIS.gamutZoom) {
						var xy = huepi.HelperUnzoomXYforModel(THIS.xy[0],  THIS.xy[1], THIS.gamut);
						var x = xy[0]*(THIS.width - 1);
						var y = (1 - xy[1]) * THIS.height;
					} else {
						var x = THIS.xy[0] * (THIS.width - 1);
						var y = (1 - THIS.xy[1]) * THIS.height;
					}
				} else {
				...
				switch (jsc.getSliderComponent(THIS)) {
					...
					case 'e':
						var cieColor = huepi.HelperXYtoRGBforModel(THIS.xy[0], THIS.xy[1], 1.0, THIS.gamut);
						var rgb = [cieColor.Red, cieColor.Green, cieColor.Blue];
						var color1 = 'rgb(' +
			...
			function redrawSld () {
				...
				if (sldComponent) {
					...
					switch (sldComponent) {
						case 's': var yComponent = 1; break;
						case 'v': var yComponent = 2; break;
						case 'e': var yComponent = 3; break;
					}
					if (yComponent == 3) {
						var y = Math.round((1 - THIS.xy[2]) * (THIS.height - 1));
					} else {
						var y = Math.round((1 - THIS.hsv[yComponent] / 100) * (THIS.height - 1));
					}
