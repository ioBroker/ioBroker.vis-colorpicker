...
huepi.HelperHuetoHueAng = function (Hue) {
    ...
    if (Ang > 0) {
        ...
        else if (Ang < 180) Ang = Ang - (180 - Ang) * 0.17 * ((180 - Ang) / 90); // Longer Red, shorter Green
    }
    return Ang;
};
...
huepi.HelperUnzoomXYforModel = function (Zx, Zy, Model) {
    ...
    return [
        Math.max(
            ...
            Math.min(
                1.0,
                (Zx - Math.max(0, PBlue.x - 0.05)) / (Math.min(1, PRed.x + 0.05) - Math.max(0, PBlue.x - 0.05)),
            ),
        ),
        Math.max(
            0.0,
            Math.min(
                1.0,
                (Zy - Math.max(0, PBlue.y - 0.05)) / (Math.min(1, PGreen.y + 0.05) - Math.max(0, PBlue.y - 0.05)),
            ),
        ),
    ];
};
...
huepi.HelperColortemperaturetoRGB = function (Temperature) {
    ...
    if (Temperature <= 66) Red = /*255*/ 165 + 90 * (Temperature / 66);
    else {
        Red = Temperature - 60;
