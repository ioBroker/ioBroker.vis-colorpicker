 ...
 * vis-1 widget set (`widgets/colorpicker.html` and `widgets/colorpicker/**`), which is maintained by hand and
...
const TARGET = 'widgets/vis-2-widgets-colorpicker';

function copyAllFiles() {
    copyFiles(['src-widgets/build/**/*', '!src-widgets/build/index.html'], `${TARGET}/`);
}

/** Keeps the version and the copyright year in the vis-1 widget set in sync with package.json */
function syncLegacyVersion() {
    const pack = require('./package.json');
    const year = new Date().getFullYear();

    const file = 'widgets/colorpicker.html';
    const content = fs.readFileSync(`${__dirname}/${file}`, 'utf8');
    const updated = content
        .replace(/version: "\d+\.\d+\.\d+"/g, `version: "${pack.version}"`)
        .replace(/version: '\d+\.\d+\.\d+',/g, `version: '${pack.version}',`)
        .replace(/Copyright (\d{4})-\d{4}/g, `Copyright $1-${year}`);
    if (content !== updated) {
